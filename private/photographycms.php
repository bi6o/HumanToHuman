...
$output = "";
   ...
   $output .= '<form name="submit" action="photographycms.php" method="post">
        ...
        <a href="delete_photography.php?photography='.urlencode($id).'" onclick="return confirm("Are You Sure?");" >  Delete</a>
        </form>';
                    }
  $output .= '<form name="submit" action="photographycms.php" method="post">
         
        <div class="cmsalbum col-sm-9">
                          <h3 class="col-sm-offset-5">New Photography item</h3>
...
if (isset($_POST['new']))
         {  
            ...
            $query = "INSERT INTO photography (`title`,`content`,`photo`,`caption`) values ('".$newtitle."','".$newcontent."','".$newphoto."','".$newcaption."')";
            file_put_contents('test.txt',$query);
            $result = mysqli_query($connection , $query); //this is excuted in case of insert and delete and update as well
            //test if there is query error
            if ($result && mysqli_affected_rows($connection)>=0)
            {
                $_SESSION['message'] = "photography item Created";
                 
               redirect_to('manage_content.php');
            }
            else
            {
                $_SESSION['message']= "New photography failed";
            }
